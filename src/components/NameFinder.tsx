...
export function NameFinder() {
  ...
  const matchingNames = useMemo(() => {
    ...
    return Object.entries(polishNames).filter(([formal, data]) => {
      return formal.toLowerCase().includes(term) ||
        data.diminutives.some(dim => dim.toLowerCase().includes(term));
    });
  }, [searchTerm]);
    ...
    <div className="container px-4 sm:px-6 py-6 sm:py-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {matchingNames.length === 0 ? (
          ...
          matchingNames.map(([formal, data]) => (
            <NameCard key={formal} formal={formal} data={data}/>
          ))
        )}
      </div>
    </div>
